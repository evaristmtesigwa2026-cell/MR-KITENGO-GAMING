                ...
                card.innerHTML = `
                    <div class="admin-card-wrapper" data-cat="${catId}" data-key="${key}">
                        ...
                        <div style="display: flex; flex-wrap: wrap; gap: 8px; margin-top:10px;">
                            <button onclick="window.openEditLinkModal('${catId}', '${key}', 'setLink', 'TIKTOK LINK')" style="flex: 1; min-width:90px; background-color: #000000; border: 1px solid #ff007f; padding:6px; font-size:11px; min-height:36px;">TIKTOK LINK</button>
                            <button onclick="window.openEditLinkModal('${catId}', '${key}', 'link', 'GAME LINK')" style="flex: 1; min-width:90px; background-color: #7139e8; padding:6px; font-size:11px; min-height:36px;">GAME LINK</button>
                            <button onclick="window.openEditLinkModal('${catId}', '${key}', 'password', 'PASSWORD')" style="flex: 1; min-width:90px; background-color: #ff8800; padding:6px; font-size:11px; min-height:36px;">PASSWORD</button>
                        </div>
...
window.openEditLinkModal = function(catId, key, field, label) {
    window.showLoader("LOADING...");
    database.ref(`buses/${catId}/${key}/${field}`).once('value')
    .then((snapshot) => {
        const currentValue = snapshot.val() || '';
        document.getElementById("edit-link-modal-title").textContent = "HARIRI " + label;
        document.getElementById("edit-link-old-value").value = currentValue ? currentValue : "(Haijawekwa bado)";
        document.getElementById("edit-link-new-value").value = "";
        document.getElementById("edit-link-catid").value = catId;
        document.getElementById("edit-link-key").value = key;
        document.getElementById("edit-link-field").value = field;
        document.getElementById("edit-link-modal-screen").style.display = "flex";
        window.hideLoader();
    })
    .catch(err => {
        alert('Kosa: ' + err.message);
...
window.closeEditLinkModal = function() {
    document.getElementById("edit-link-modal-screen").style.display = "none";
};

window.saveEditLinkModal = function() {
    const catId = document.getElementById("edit-link-catid").value;
    const key = document.getElementById("edit-link-key").value;
    const field = document.getElementById("edit-link-field").value;
    const newValue = document.getElementById("edit-link-new-value").value.trim();

    if (newValue === "") { alert("Tafadhali weka thamani mpya kabla ya kuhifadhi!"); return; }

    window.showLoader("LOADING...");
    database.ref(`buses/${catId}/${key}/${field}`).set(newValue)
    .then(() => {
        alert('Imehifadhiwa kwa ufanisi!');
