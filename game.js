...
// Kuanzisha Firebase (na uhakiki wa kama SDK imepakiwa vizuri)
let database;
try {
    firebase.initializeApp(firebaseConfig);
    database = firebase.database();
} catch (err) {
    console.error("Kosa la kuanzisha Firebase:", err);
    alert("Tatizo la mtandao: mfumo umeshindwa kuunganishwa na database. Tafadhali refresh ukurasa.");
}

// Kizuizi rahisi cha HTML Injection - hutumika kila data inayotoka kwa mtumiaji
// inapoingizwa kwenye innerHTML, ili kulinda muundo wa ukurasa
...
// SHA-256 hashing kwa ajili ya kuhifadhi password kwa usalama zaidi (badala ya maandishi wazi)
...
window.register = async function() {
    ...
    if (name == "" || email == "" || password == "") { alert("Jaza nafasi zote!"); return; }
...
window.login = async function() {
    ...
    if (name == "" || password == "") { alert("Jaza nafasi zote!"); return; }

    const hashedInput = await window.hashPassword(password);
    if (name == dbname && hashedInput == dbpassword) {
...
window.compressImage = function(file, maxWidth, maxHeight, quality, callback, onError) {
    const reader = new FileReader();
    reader.onerror = function() {
        if (onError) onError("Imeshindwa kusoma picha uliyochagua.");
    };
    reader.onload = function(event) {
        const img = new Image();
        img.onerror = function() {
            if (onError) onError("Faili ulilochagua sio picha sahihi.");
        };
        img.onload = function() {
    ...
    database.ref('categories').once('value').then((snapshot) => {
        ...
        Object.keys(data).forEach((id) => {
            ...
            if (container) {
                ...
                card.className = 'card';
                const safeName = window.escapeHtml(cat.name);
                card.innerHTML = `
                    <p>${safeName}</p> 
                    <img src="${cat.image}" alt="${safeName}" loading="lazy" style="width: 100%; height: 115px; border-radius: 12px; object-fit: cover; cursor: pointer; border: 1px solid rgba(255,255,255,0.1);"> <br>
                    <button onclick="window.showBusCategory('${id}', '${safeName}')">CHAGUA HAPA</button>
    ...
    database.ref('buses/' + categoryId).once('value').then((snapshot) => {
        ...
        Object.keys(busesData).forEach((key) => {
            ...
            card.innerHTML = `
                ...
                <p>${safeName} ${isPremium ? `<span style='color:#ff007f;font-size:14px;display:block;margin-top:5px;font-family:"Orbitron", sans-serif;text-shadow: 0 0 5px rgba(255,0,127,0.3);'>(Tsh ${item.price})</span>` : ''}</p>
                <img src="${item.image}" alt="${safeName}" loading="lazy" class="bus-thumb" style="width: 100%; height:115px; border-radius:12px; object-fit:cover; cursor: pointer; border: 1px solid rgba(255,255,255,0.1);"><br><br>
                
                ${isPremium ? 
                  `<button class="btn-buy">BUY MOD</button>` : 
                  `<button><a href="${item.link}" target="_blank" rel="noopener">DOWNLOAD</a></button>`
                }
                
                <button class="btn-delete" id="del-bus-${key}" style="display:none;" onclick="window.deleteBus('${categoryId}', '${key}')">FUTA</button>
            `;

            card.querySelector(".bus-thumb").addEventListener("click", () => {
...
window.uploadBus = function() {
    ...
    if (price && isNaN(parseInt(price))) { alert("Bei uliyoweka sio namba sahihi!"); return; }
...
// =========================================================================
// KITENGO AI ASSISTANT
// ONYO LA USALAMA: API key hapa chini bado inaonekana kwa mtu yeyote
// atakayefungua "View Source" ya ukurasa huu. Njia salama zaidi ni kuipitisha
// kwenye backend (mfano Netlify Function) ili isionekane kabisa upande wa
// mtumiaji. Nipo tayari kukusaidia kuweka usanidi huo wakati wowote.
// =========================================================================
...
window.sendMessage = async function() {
    ...
    try {
        ...
        const aiResponseText = data?.candidates?.[0]?.content?.parts?.[0]?.text || "Samahani mkuu, sijapata jibu sahihi kwa sasa.";
