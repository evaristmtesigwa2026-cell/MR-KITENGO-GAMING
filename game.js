    ...
    database.ref('categories').once('value', (snapshot) => {
        ...
        for (const [key, cat] of Object.entries(categories)) {
            ...
            card.innerHTML = `
                ...
                <button onclick="window.showBusCategory('${key}', '${cat.name}', false, true)" style="width: 100%; margin-top: 15px;">CHAGUA HAPA</button>
            `;
...
window.showBusCategory = function(catId, catName, isAdminMode = false, forceAdminView = false) {
    database.ref('buses/' + catId).once('value', (snapshot) => {
        ...
        // Check if user is admin (by checking if hash is #admin)
        let isAdmin = window.location.hash === "#admin" || forceAdminView;
...
window.reloadCategoryView = function(catId, catName) {
    window.showBusCategory(catId, catName, false, true);
}
